...
const ChannelContextProvider = ({ children }) => {
  ...
  const { currentChannel, setCurrentChat, setCurrentChannel } =
  ...
  const viewModelToDb = (user, msg, type) => {
    ...
    if (type === "[JOIN]") chat.isJoining = true;
    else chat.isJoining = false;
    return chat;
  };

  const addChannelLog = async (
    channel,
    user,
    msg,
    type
